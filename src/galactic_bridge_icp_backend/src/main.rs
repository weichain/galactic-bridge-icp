use candid::Nat;
use galactic_bridge_icp_backend::address::{
    validate_address_as_destination, AddressValidationError,
};
use galactic_bridge_icp_backend::deposit::scrap_eth_logs;
use galactic_bridge_icp_backend::endpoints::events::{
    Event as CandidEvent, EventSource as CandidEventSource, GetEventsArg, GetEventsResult,
};
use galactic_bridge_icp_backend::endpoints::{
    Eip1559TransactionPrice, GasFeeEstimate, MinterInfo, RetrieveEthRequest, RetrieveEthStatus,
    WithdrawalArg, WithdrawalError,
};
use galactic_bridge_icp_backend::eth_logs::{EventSource, ReceivedEthEvent};
use galactic_bridge_icp_backend::guard::retrieve_eth_guard;
use galactic_bridge_icp_backend::lifecycle::MinterArg;
use galactic_bridge_icp_backend::logs::{DEBUG, INFO};
use galactic_bridge_icp_backend::memo::BurnMemo;
use galactic_bridge_icp_backend::numeric::{LedgerBurnIndex, Wei};
use galactic_bridge_icp_backend::state::audit::{process_event, Event, EventType};
use galactic_bridge_icp_backend::state::transactions::{EthWithdrawalRequest, Reimbursed};
use galactic_bridge_icp_backend::state::{
    lazy_call_ecdsa_public_key, mutate_state, read_state, State, STATE,
};
use galactic_bridge_icp_backend::withdraw::{
    process_reimbursement, process_retrieve_eth_requests, CKETH_WITHDRAWAL_TRANSACTION_GAS_LIMIT,
};
use galactic_bridge_icp_backend::{
...
fn post_upgrade(minter_arg: Option<MinterArg>) {
    use galactic_bridge_icp_backend::lifecycle;
    match minter_arg {
...
fn is_address_blocked(address_string: String) -> bool {
    ...
    galactic_bridge_icp_backend::blocklist::is_blocked(address)
}
...
fn get_events(arg: GetEventsArg) -> GetEventsResult {
    ...
    fn map_event(Event { timestamp, payload }: Event) -> CandidEvent {
        use galactic_bridge_icp_backend::endpoints::events::EventPayload as EP;
        CandidEvent {
            timestamp,
...
fn http_request(req: HttpRequest) -> HttpResponse {
    ...
    if req.path() == "/metrics" {
        ...
        fn encode_metrics(w: &mut MetricsEncoder<Vec<u8>>) -> std::io::Result<()> {
            ...
            read_state(|s| {
                ...
                galactic_bridge_icp_backend::eth_rpc::encode_metrics(w)?;
    ...
    // TODO: askama/askama_derive issue
    ...
    //         .build()
        ...
        use galactic_bridge_icp_backend::logs::{Log, Priority, Sort};
        use std::str::FromStr;
...
#[query]
fn check_audit_log() {
    use galactic_bridge_icp_backend::state::audit::replay_events;
